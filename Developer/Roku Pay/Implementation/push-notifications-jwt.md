...
See Setting up Roku Pay web services for how to add production and test push notification endpoints and enable them to receive secured Roku Pay push notifications.
...
HTTP body
Roku Pay push notifications use the JavaScript Object Signing and Encryption (JOSE) framework, which relies on a JSON Web Token (JWT) to deliver messages securely. The HTTP body of a Roku Pay push notification, which adheres to the JWS compact serialization specification , includes the following JSON structures:
JOSE header . Specifies the key ID, which indicates the public key to be used to verify the signature and the signing algorithm required to decode the public key.
...
The JSON format of the JWS signature conforms to the JWS RFC 7515 standard.
...
Decode each string.
Use the public key to verify the signature string against the contents of the period-concatenated header and payload strings.
Decode the x-Roku-message JWT claim within the payload to get the contents of the notification .

Publishers can visit https://jwt.io , go to the Debugger section, and try to decode the push notifications.
Decoding the x-Roku-message JWT claim to get the push notification message
The x-Roku-message field within the JWT claim payload is a base64url-encoded string that contains the contents of the push notification message. The following example demonstrates the decoded x-Roku-message field for a Sale notification message :
{
...
See the Roku Pay push notifications reference for more information on the contents of the different Roku Pay push notification messages.
...
Public keys
Publishers must use Roku-provided public keys, which are located here , to verify message signatures. The public keys are provided in JSON Web Key (JWK) format and are similar to the example shown below. Select the key that corresponds to the kid specified in the JOSE header .
...
Publishers must get the public keys from the specified URL . Notifications with a different URL in the JOSE header must be rejected.
...
Testing
Prior to enabling JWT signature authentication on a production endpoint, publishers can configure a test endpoint in the Developer Dashboard and verify whether it can receive and process the JWT/JWS-secured messages. See Setting up Roku Pay web services for more information.
In addition, publishers can manually send a test JWT/JWS-secured message with a generic payload to the configured test push notification endpoint. This enables publishers to verify that their test endpoint can receive a JWT/JWS-secured message without generating Roku Pay transactions. See Setting up Roku Pay web services for more information.
Sample
You can download Roku's sample application , use it as a template for your implementation, and update it with your custom business logic for processing Roku Pay push notifications. Your backend system application must communicate via HTTPS or leverage an HTTPS reverse proxy.