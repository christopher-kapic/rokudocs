...
You can combine video content and ad pods into a single playlist and then render it. This optimizes the video playback experience by eliminating buffering between your content and ads. The Roku Advertising Framework (RAF) API includes constructStitchedStream() and renderStitchedStream() methods that enable you to seamlessly create and render stitched streams.
Creating a stitched stream
Once you have enabled the RAF library in the manifest and instantiated it in the app, you can create a stitched stream using the constructStitchedStream() method. This method takes two parameters: a ContentNode with the content metadata of the video to be played, and the ads in the ad pod to be rendered during playback (retrieved via the RAF GetAds() method ). The method returns a single playlist, which you can then render as described in the next section.
...
You can render a stitched stream using the renderStitchedStream() method. This method takes the playlist created via the constructStitchedStream() method and a renderable node to which the stitched stream can be attached.
...
Sample app
You can download and install a sample app that demonstrates how to create and render a client-side stitched stream. It shows you how you can create a stream by combining a video content item with the ads in a Video Ad Serving Template (VAST), and then render the stitched stream in a SceneGraph object.