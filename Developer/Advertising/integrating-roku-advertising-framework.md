...
analytics API. The API Reference provides
more detailed information on the video ad API, and the Use
Cases section offers
...
The following line must be placed in the manifest file for
...
(You may wish to check URL Parameter Macros to
see if any parameter values in the ad URL should be replaced with the
provided macros.)
Aside from the Configuration interface,
...
constructed as described in Getting Started .
...
available in the adPods Ad Structure .
...
Example
For examples, see the Roku Advertising sample apps
...
See the RAF API reference guide for more information on this method.
...
To enable ad measurement, call the enableAdMeasurements() method, and pass the required content metadata within the setContentGenre() , setContentId() , and setContentLength() methods.
...
Each ad server may require different Roku genre tags tags that you need to pass. Check with your ad server for the Roku genre tags to be used.
Passing the kidsContent flag with the content genre
When specifying the content genre with the setContentGenre() method, pass the kidsContent flag to specify whether the content being played is targeted towards children (true) or not (false).
...
screen, setAdBufferScreenLayer() allows
the same content meta-data attributes
as setAdBufferScreenContent() ,
...
see CustomBufferScreenSceneGraphSample .
...
Structure .
Scheduling and rendering can then proceed as described in Client Side Ad Insertion by
first calling
the importAds() method
...
single call
to getAds() to
get the entire Ad Structure . The
...
elements).
As an example, if ad contains the Ad structure for a
...
Obtain the user agent from a client-side call made to the ad stitcher or ad server.
...
server-stitched would need to be rendered by the application. See Custom Ad Rendering above.
...
relative to the current ad. Example: For a 30-second ad that starts at 15:00 in the stitched
...
the stitchedAdsInit() method with the ad structure constructed by the
client:
...
specification described in stitchedAdHandledEvent() ). In the event
