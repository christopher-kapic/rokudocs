...
content.
Simple_Grid_and_Video
Simple_Grid_with_Details_and_Video
...
sequence:
Set the content field to a ContentNode containing a
child ContentNode for each media item in the playlist.
Set the contentIsPlaylist field to true.
...
video playback scheme such as HLS, as described in Streaming specifications .)
...
of BIF files for the video. For this approach, see our documentation on BIF file creation using the Roku BIF tool .
The second option is to provide trickplay images in DASH or HLS, using the respective industry standards. You can learn more about this approach in our trickplay implementation documentation for HLS and DASH .
...
play attributes described in Content Meta-Data (see Content Meta-Data ).
...
(see Configuring video stream buffering ).
...
See SceneGraphCaptionsDemo for
...
to use the attributes listed in Content Meta-Data (see Content Meta-Data ) which are all recognized as field names for
...
attributes listed above. See Roku Search for complete details.
Video configuration Meta-Data
See Content Meta-Data for the complete list of
video configuration meta-data.
...
Simple_Grid_and_Video	Video player in a simple grid UI.
Simple_Grid_with_Details_and_Video	Video player in a simple grid UI with details page.
PanelSet_and_Video	Video player in a simple PanelSet UI.
VideoList	Video player with content selected from a LabelList.
FastVideoStart	Sample demonstrating how to use Fast Video Start.
SceneGraphCaptionsDemo	Sample demonstrating how to integrate closed caption support.