...
IDs currently supported by Roku. The list of these supported locales can be found in the description of the ifDeviceInfo GetCurrentLocale() method.
...
Roku. The string localization files should be placed in the pkg:/locale/ subdirectory for the supported locale. For example, if
you want to include a translations.ts localization file in the TS
format for Canadian French, place the file in the pkg:/locale/ subdirectory named for the locale ID fr_CA :
...
<script> element, you can use the tr() function to localize a string, if you have provided a translation of the
string in a translations.xml file in the XLIFF XML format in the
package directory pkg:/locale/locale_ID/ directory. The tr() function is a global BrightScript utility function that looks for the
...
The tr() function will look for the pkg:/locale/locale_ID/translations.xml file, and look in the file for
...
and assigned to the text field of the Label node object m.greetinglabel . If no
...
use the following format for the value of the uri field of the Poster node:
...
This causes the application to search through the pkg:/locale directory in the following order to find the localized graphical image.
...
In Brightscript in a <script> element, you can also use the getLocalizedAsset() method in ifLocalization to get a localized
graphical image.
For example:
...
TS file example
The following is an example of a translations.ts file in the TS XML format placed in the pkg:/locale/fr_CA directory to translate English to Canadian
French:
...
XLIFF file example
The following is an example of a translations.xml file in the XLIFF XML format placed in the pkg:/locale/fr_CA directory to translate English to Canadian
French:
...
Localizing content
Apps can identify the region associated with a customer's Roku account to determine which content to display in the app UI. For example, apps can get the external IP address of a Roku device client-side, and return it back to the app's backend server. In the backend, the app can get the country ranges mapped to the IP address in order to determine which content to show to the customer and allow streaming. See Channel localization for how publishers should consider localization in the development, distribution, engagement, and monetization of their apps