...
Version 3.1 – 7/2022
For apps that collect explicit in-app consent for ad targeting (for example, to adhere to GDPR), a new setLimitAdTracking() function that specifies the value of the ROKU_ADS_LIMIT_TRACKING URL parameter macro to be passed into beacons and ad requests
...
Improvements to Roku ad watermark feature
...
Extended Roku ad watermark to support Demand API calls
...
New Feature: Methods for accessing Roku Demand API . Provides more efficient monetization by connecting to real-time ad demand, while preserving control of inventory allocation with publisher's ad server
...
Multitude of bug fixes and enhancements, notably stitchedAdsInit() now clears the ad badge if called mid-ad break ( early cue-in use case)
...
New enableInPodStitching(isIPS as Boolean) method brings benefits from CSAS API to apps using the classic CSAI showAds() by stitching together multiple video clips within a single ad break (no buffering between ads)
When parsing VAST, preserve the id attribute of <MediaFile/>
Added support for getAds() parsing a local file from tmp:/ via e.g. setAdURL("tmp:/myVASTorVMAPorSMRX.xml")
Multitude of bug fixes and enhancements
Deployed to devices on Roku OS 9.4 and above

Version 2.13 – 7/2020
Multitude of bug fixes and enhancements, including Reduce JIT pre-fetching to improve playback start time for client-stitched use case Fix regression of fireRokuMarketingPixel() not URL-encoding its arguments Fix errors on bad metadata (e.g. when parsing VAST, getting a single ad with yet having fallback_index>0)
...
Added handling of voice ETC for ads
Interactive ads improvements
Bug fixes and performance improvements
Deployed to devices with Roku OS 9.1 and above

Version 2.9 – 06/2019
Added a ROKU_ADS_LOCALE macro which returns current locale in same format
as roDeviceInfo.getCurrentLocale() (e.g. "en_US", "es_ES")
Library manifests
internally rsg_version=1.2 ,
...
Innovid interactive ads) New view parameter for showAds() , which is required for all SceneGraph applications
...
Version 1.9 – 11/2016
Freewheel SmartXML adReplica changes Improve forecasting by only resolving ad requests for wrapped
creative renditions that are placed into ad slots Respect replicaId if specified in the adReference tag and a matching
