...
The screensaver's BrightScript Debugger uses port 8087.
Discovery
...
You can access the list of available screensavers in the screensaver store or menu that is discoverable on your device (in the Roku UI) by going to Settings > Theme > Screensavers . This is where you can go to get, set, or remove screensavers. Note that once a screensaver is purchased and added
...
Manifest entries
A screensaver must have an entry in its manifest file named screensaver_title whose value is the title of the screensaver (usually the same as the title of the app).
