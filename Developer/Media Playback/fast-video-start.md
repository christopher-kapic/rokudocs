...
screen before actual playback, while the video stream is buffering. "Fast start" allows this video stream buffering to take place before the user has actually selected a video for playback, by
...
Prebuffer() has no
effect.
SceneGraph apps use the Video node control field prebuffer option to implement Fast Video Start. See Fast start media playback for information on implementing Fast Video Start in a SceneGraph app.
