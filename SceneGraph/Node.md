...
id	string	""	READ_WRITE	Adds a dictionary entry that allows the node to be retrieved with ifSGNodeDict findNode() function.
...
insert	A child node was inserted at index1. If multiple child nodes were inserted (for example, via the insertChildren() function), the last inserted child node is stored at index2.
add	A child node was added to the end of the children node tree (at index 1). If multiple child nodes were added (for example, via the appendChildren() function), the last added child node is stored at index2.
