...
Also review " SceneGraph threads " for in-depth information on using Task nodes most efficiently.
Example
...
In addition to this example, a much simpler, complete example, including the scene that creates the Task node, can be found here: SimpleTask example . That example simply uses a Task node to increment a counter and display the current counter value on the screen.
Fields
...
Sample app
TaskExample is a sample app demonstrating Task in action.