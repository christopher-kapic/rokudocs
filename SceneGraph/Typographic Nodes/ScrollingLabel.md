...
height	float	0	READ_WRITE	Specifies the height of the label. If set to zero, the actual height is determined by the value of the numLines field if it is greater than zero. See Vertical Alignment
...
horizAlign	option string	left	READ_WRITE	See Horizontal Alignment
vertAlign	option string	top	READ_WRITE	See Vertical Alignment

Sample app
ScrollingLabelExample is a sample app demonstrating ScrollingLabel in action.