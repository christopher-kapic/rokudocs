...
Description
Returns the bounding rectangle for scene components (component nodes extended from a Scene or OverhangPanelSetScene node class).
...
If the node is not connected to a Scene node, this will return the same value as the boundingRect() method.
Parameters
