...
roku815px - expressionsvariables1
...
roku815px - expressionsvariables2
...
roku815px - expressionsvariables3
...
If a constant contains 10 or more digits, or if D is used in the exponent, that number is double precision. Adding a # declaration character also forces a constant to be double precision.

If the number is not double-precision, and if it contains a decimal point, then the number is float. If the number is expressed in exponential notation with E preceding the exponent, the number is float.

If neither of the above is true of the constant, then it is an integer.
